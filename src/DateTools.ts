...
export const createDate = (dateString: string): string => {
...
export const formatDefaultDate = (dateObject: Date): string => {
  let year = dateObject.getFullYear().toString();
  let month = formatValue(dateObject.getMonth() + 1);
  let day = formatValue(dateObject.getUTCDate());
  return `${year}-${month}-${day}`;
...
export const formatDateToTable = (dateObject: Date): string => {
  let year = dateObject.getFullYear().toString();
  let month = formatValue(dateObject.getMonth() + 1);
  let day = formatValue(dateObject.getUTCDate());
